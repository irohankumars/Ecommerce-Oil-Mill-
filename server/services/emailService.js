...
export function sendWelcomeEmail(user) {
  return sendMail({ to: user.email, subject: "Welcome to Velora", text: `Welcome ${user.name}. Your Velora account is ready.`, html: htmlLayout("Welcome to Velora", `<p>Welcome ${user.name}. Your account is ready for fresh cold pressed oil orders.</p>`) });
}

export function sendVerificationEmail(user, token) {
  const url = `${env.clientUrl}/auth/verify-email/${token}`;
  return sendMail({ to: user.email, subject: "Verify your Velora account", text: `Verify your email: ${url}`, html: htmlLayout("Verify your email", `<p>Confirm your email to secure your Velora account.</p><p><a href="${url}" style="color:#4f6f2f;font-weight:700">Verify email</a></p>`) });
}

export function sendPasswordResetEmail(user, token) {
  const url = `${env.clientUrl}/auth/reset-password/${token}`;
  return sendMail({ to: user.email, subject: "Reset your Velora password", text: `Reset your password: ${url}`, html: htmlLayout("Reset your password", `<p>Use this secure link to reset your password.</p><p><a href="${url}" style="color:#4f6f2f;font-weight:700">Reset password</a></p>`) });
}

export function sendOtpEmail(user, code, purpose) {
  return sendMail({ to: user.email, subject: "Velora security code", text: `Your ${purpose} code is ${code}. It expires shortly.`, html: htmlLayout("Security code", `<p>Your ${purpose} code is:</p><p style="font-size:28px;font-weight:800;letter-spacing:4px">${code}</p><p>This code expires shortly.</p>`) });
}
...
export function sendContactFormEmail(message) {
  ...
  return sendMail({ to: env.email.contactTo, replyTo: message.email, subject: `Velora contact: ${message.subject || "New message"}`, text: `${message.name} <${message.email}>\n${message.phone || ""}\n\n${message.message}` });
}